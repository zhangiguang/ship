...
##########################
    ...
    # box_diff = bbox_pred - bbox_targets
    # ######
    # # shape = tf.shape(box_diff)[-1]
    # shape0 = tf.shape(box_diff)[0]
    # shape1 = tf.shape(box_diff)[1]
    # #box_diff_like = tf.ones_like(box_diff)
    # zeros = tf.zeros([shape0, shape1, 4])
    # ones = tf.ones([shape0, shape1, 1])
    #
    # zeros_ones = tf.concat([zeros, ones], axis=2)
    # oneslike = tf.ones_like(box_diff)
    # box_diff_like = tf.add(zeros_ones, oneslike)
    # box_diff = tf.multiply(box_diff, box_diff_like) ### theta's weight *2
    # #box_diff = tf.where(tf.equal(shape, 4), tf.multiply(box_diff, box_diff_like), box_diff)
    #
    # abs_box_diff = tf.abs(box_diff)
    #
    # smoothL1_sign = tf.stop_gradient(
