...
os.environ["CUDA_VISIBLE_DEVICES"] = cfgs.GPU_GROUP#'1'
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session(config=config) as sess:
            ...
            for i in range(img_num):
                ...
                cv2.imwrite(cfgs.TEST_SAVE_PATH + str(_img_name_batch[0]) + '_horizontal_fpn.jpg', _img_batch_fpn_horizonal)
                cv2.imwrite(cfgs.TEST_SAVE_PATH + str(_img_name_batch[0]) + '_rotate_fpn.jpg', _img_batch_fpn_rotate)
                ...
                cv2.imwrite(cfgs.TEST_SAVE_PATH + str(_img_name_batch[0]) + '_horizontal_gt.jpg', _img_batch_gt_horizontal)
                cv2.imwrite(cfgs.TEST_SAVE_PATH + str(_img_name_batch[0]) + '_rotate_gt.jpg', _img_batch_gt_rotate)
                ...
                for m in range(len(_gtboxes_and_label)):
                    for n, box in enumerate(_fast_rcnn_decode_boxes_rotate):
                        ...
                        gt_yc, gt_xc, gt_h, gt_w, gt_theta = gt_box[0], gt_box[1], gt_box[2], gt_box[3], gt_box[4]
                ...
                if(len(_fast_rcnn_decode_boxes_rotate)>0):
                    FalseAlarm += len(_fast_rcnn_score_rotate) - Current_TrueDetection
            ...
            print("Op:  " + str(TrueDetection) + "/" + str(TotalObjects) + "=" + str(Op))
