...
def get_file_paths_recursive(folder=None, file_ext='.png'):
    ...
    with tf.Session(config=config) as sess:
        ...
        for count, img_path in enumerate(file_paths):
            ...
            for sub_class in range(1, cfgs.CLASS_NUM + 1):
                ...
                tmp_boxes_r = tmp_boxes_r[tmp_score_r > 0.4]
                tmp_score_r = tmp_score_r[tmp_score_r > 0.4]
                if not tmp_boxes_r.any():
                    continue
                if len(tmp_boxes_r.shape) == 1:
                    tmp_boxes_r = np.expand_dims(tmp_boxes_r, 0)
                ...
                #                                     iou_threshold=0.2, #threshold[LABEl_NAME_MAP[sub_class]],
                #                                     max_output_size=500)
                # except:
                #     # Note: the IoU of two same rectangles is 0, which is calculated by rotate_gpu_nms
                #     jitter = np.zeros([tmp_boxes_r.shape[0], tmp_boxes_r.shape[1] + 1])
                #     jitter[:, 0] += np.random.rand(tmp_boxes_r.shape[0], ) / 1000
                #     inx = rotate_gpu_nms(np.array(tmp, np.float32) + np.array(jitter, np.float32),0.2, 0)
...
def parse_args():
    ...
    parser.add_argument('--src_folder', dest='src_folder',
                        help='/media/iguang/23a6d0dc-b99b-49a8-9ea9-c857f767ba3a/ship/',
...
if __name__ == "__main__":
    ...
    file_paths = get_file_paths_recursive('/media/iguang/23a6d0dc-b99b-49a8-9ea9-c857f767ba3a/ship/images/', 'png')
    #file_paths = os.path.join('/media/iguang/23a6d0dc-b99b-49a8-9ea9-c857f767ba3a/ship/', 'us_nfk.jpg')
    ...
    inference(det_net, file_paths, '/media/iguang/23a6d0dc-b99b-49a8-9ea9-c857f767ba3a/ship/', 1024, 1024,
               400, 400,  False)
