...
def parse_rec(filename):
  ...
  for obj in tree.findall('object'):
    ...
    rbox = [int(float(bbox.find('x1').text)), int(float(bbox.find('y1').text)), int(float(bbox.find('x2').text)),
            int(float(bbox.find('y2').text)), int(float(bbox.find('x3').text)), int(float(bbox.find('y3').text)),
            int(float(bbox.find('x4').text)), int(float(bbox.find('y4').text)), int(bbox.find('label').text)]
...
def eval_with_plac(img_dir, det_net, image_ext, draw_imgs=False):
    ...
    with tf.Session(config=config) as sess:
        ...
        for i, a_img_name in enumerate(imgs):
            ...
            recs = {}
            recs[a_img_name] = parse_rec(os.path.join(test_annotation_path, a_img_name + '.xml'))
            ...
            keep = nms_rotate.nms_rotate_cpu(det_boxes_r_,det_scores_r_,0.3, 20)
            det_boxes_r_ = det_boxes_r_[keep]
            det_scores_r_ = det_scores_r_[keep]
            det_category_r_ = det_category_r_[keep]
            ##### box_ratio > 2  or < 1/2
            index = (det_boxes_r_[:, 2] / det_boxes_r_[:, 3] > 2) | (det_boxes_r_[:, 2] /det_boxes_r_[:, 3] <= 0.5 )
            ...
            if draw_imgs:
                det_detections_h = draw_box_in_img.draw_rotate_box_cv1(np.squeeze(resized_img, 0),
                                                               boxes=bbox,
                                                               labels=labels,
...
def eval(img_dir, image_ext, test_annotation_path):
    ...
    faster_rcnn = build_whole_network_RROI.DetectionNetwork(base_network_name=cfgs.NET_NAME,
                                                       is_training=False)
    eval_with_plac(img_dir=img_dir, det_net=faster_rcnn, image_ext=image_ext, draw_imgs=True)
...
def parse_args():
    ...
    parser.add_argument('--img_dir', dest='img_dir',
                        help='images path',
                        default='/media/iguang/23a6d0dc-b99b-49a8-9ea9-c857f767ba3a/ship/VOC2007(test)/JPEGImages', type=str)
...
if __name__ == '__main__':
    ...
    img_dir = '/media/iguang/23a6d0dc-b99b-49a8-9ea9-c857f767ba3a/ship/VOC2007(test)/JPEGImages'
    image_ext = '.jpg'
    test_annotation_path = '/media/iguang/23a6d0dc-b99b-49a8-9ea9-c857f767ba3a/ship/VOC2007(test)/Annotations'
    os.environ["CUDA_VISIBLE_DEVICES"] = '0'

    eval(img_dir, image_ext, test_annotation_path)
