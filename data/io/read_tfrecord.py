...
def read_single_example_and_decode(filename_queue):
    ...
    features = tf.parse_single_example(
        ...
        features={
            ...
            'img': tf.FixedLenFeature((), tf.string, default_value=''), #[], tf.string),
    ...
    img = tf.image.decode_png(features['img'])
    ...
    gtboxes_and_label = tf.decode_raw(features['gtboxes_and_label'], tf.int32) #将编码为字符串类型的变量重新变回来。制作图片源数据一般写进tfrecord里用to_byte格式，也就是字符串，这里将原始数据取出来就必须制定原始数据的格式
...
def next_batch(dataset_name, batch_size, shortside_len, is_training):
    if dataset_name not in ['OPship', 'USship', 'spacenet', 'pascal', 'DOTA', 'Optical_ship', 'Optical_GG01_ship']:
        raise ValueError('dataSet name must be in pascal or coco')

    if is_training:
        pattern = os.path.join('../data/tfrecords', dataset_name + '_train*')
        #pattern = os.path.join('/media/iguang/0004B05B000AB2F6', 'USship_train.tfrecord')
    else:
