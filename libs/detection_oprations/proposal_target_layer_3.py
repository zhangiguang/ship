...
def proposal_target_layer_3(rpn_rois, rois_h, gt_boxes_h, gt_boxes_r, fast_iou_positive_threshld):
    """
    ...
    labels, rois,bbox_targets_r = _sample_rois(all_rois, rois_h, gt_boxes_h, gt_boxes_r,
...
def _sample_rois(all_rois,  rois_h, gt_boxes_h, gt_boxes_r, fg_rois_per_image,
    ...
    # ######################################################  add
    # if gt_boxes_h.shape[0] == 0:
    #     gt_boxes_h = np.zeros((1, 4))


    overlaps = bbox_overlaps(
        np.ascontiguousarray(rois_h, dtype=np.float),
