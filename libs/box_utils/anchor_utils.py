...
def make_anchors(base_anchor_size, anchor_scales, anchor_ratios,
    ...
    with tf.variable_scope(name):
        ...
        with tf.Session() as sess:
            x_centers = tf.range(featuremap_width, dtype=tf.float32) * stride  # x_center = [0, 16,32,......,1024]
            y_centers = tf.range(featuremap_height, dtype=tf.float32) * stride
#            print(x_centers.eval())
#            print(y_centers.eval())
            x_centers, y_centers = tf.meshgrid(x_centers, y_centers)    # x_center = 1024 [0, 16,32,......,1024]
#            print('--------------------------------------')                                                            # y_center = 1024 [0, 0 , ........,0]
#            print(x_centers.eval())                                                                        #      ...
#            print(y_centers.eval())                                                                              # [1024,1024,1024,....,1024]
#            print('======================================')
#            print('ws:', ws.eval())
            ws, x_centers = tf.meshgrid(ws, x_centers)
            hs, y_centers = tf.meshgrid(hs, y_centers)
#            wss = ws.eval()
#            x_centers_1 = x_centers.eval()
#            print('-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=')
#            print(wss)
#            print(x_centers_1)
...
def enum_ratios(anchors, anchor_ratios):
    ...
    ws = tf.reshape(ws / sqrt_ratios[:, tf.newaxis], [-1, 1])  #   why sqrt???
