...
from libs.losses import losses_old
...
from libs.detection_oprations.proposal_target_layer_r import proposal_target_layer_r
from libs.detection_oprations.proposal_target_layer_3 import proposal_target_layer_3
...
class DetectionNetwork(object):
    ...
    def postprocess_fastrcnn_h(self, rois, bbox_ppred, scores, img_shape, scope):
        '''
        ...
        with tf.name_scope('postprocess_fastrcnn_h_{}'.format(scope)):
    ...
    def postprocess_fastrcnn_r(self, rois, bbox_ppred, scores, img_shape, scope):
        '''
        ...
        with tf.name_scope('postprocess_fastrcnn_r_{}'.format(scope)):
            rois = tf.stop_gradient(rois)
            scores = tf.stop_gradient(scores)
    ...
    def postprocess_cascade(self, rois, bbox_ppred, scope, five = True):
        '''
        ...
        with tf.name_scope('postprocess_cascade_{}'.format(scope)):
            ...
            # decode boxes
            decoded_boxes = encode_and_decode.decode_boxes_rotate(encode_boxes=bbox_ppred,
                                                           reference_boxes=rois,
                                                           scale_factors=cfgs.ROI_SCALE_FACTORS, five=five)

        return decoded_boxes #x,y,w,h,theta
    ...
    def rotate_roi_pooling(self, feature_maps, rois, rois_rorate,  img_shape):
        '''
        Here use roi warping as roi_pooling
        ...
        with tf.variable_scope('rotate_roi_pooling'):
            img_h, img_w = tf.cast(img_shape[1], tf.float32), tf.cast(img_shape[2], tf.float32)
            N = tf.shape(rois)[0]
            x1, y1, x2, y2 = tf.unstack(rois, axis=1)
            ...
            x, y, w, h, theta = tf.unstack(rois_rorate, axis=1)

            roi_mask = boxes_utils.get_mask_tf(rois_rorate, cfgs.ROI_SIZE)
            roi_mask = tf.stack([roi_mask for _ in range(1024)], axis=3)
            crop_roi = cropped_roi_features * roi_mask # [256,14,14,1024] * [256, 14, 14]
            pad_crop_feature = tf.image.resize_image_with_crop_or_pad(crop_roi, 21, 21)
            rotate_feature = tf.contrib.image.rotate(pad_crop_feature, theta)

            roi_features = slim.max_pool2d(rotate_feature,
                                           [3, 3],
                                           stride=3)
    ...
    #########################3
    def cond(self, feature_maps, rois_h, rois, a, n ):
        return a < n
    def rotate_roi(self, feature_maps, roi):
        '''

        :param feature_maps: [-1, 64, 64, 1024]
        :      rois_h: [xmin, ymin, xmax, ymax]
        :param rois: [x, y, w, h, theta]
        :return: roi_features
        '''
        print(roi)
        roi = tf.reshape(roi, [-1,5])


        #rois_h = boxes_utils.get_horizen_minAreaRectangle(roi, with_label=False)
        rpn_proposals_boxes_convert = tf.py_func(forward_convert,
                                                 inp=[roi, False],
                                                 Tout=tf.float32)
        rpn_proposals_boxes_convert = tf.reshape(rpn_proposals_boxes_convert, [8,])


        x_list = tf.strided_slice(rpn_proposals_boxes_convert, begin=[0], end=[8],
                                  strides=[2])
        y_list = tf.strided_slice(rpn_proposals_boxes_convert, begin=[1], end=[8],
                                  strides=[2])


        xmax = tf.reduce_max(x_list, axis=0)//16
        xmin = tf.reduce_min(x_list, axis=0)//16
        ymax = tf.reduce_max(y_list, axis=0)//16
        ymin = tf.reduce_min(y_list, axis=0)//16
        #
        #
        #
        # #xmin, ymin, xmax, ymax = tf.unstack(rois_h//cfgs.ANCHOR_STRIDE)
        # #xmin, ymin, xmax, ymax = rois_h[0], rois_h[1], rois_h[2], rois_h[3],
        w = tf.cast(xmax - xmin, tf.int32)
        h = tf.cast(ymax - ymin, tf.int32)
        # # shape = tf.shape(feature_maps)
        # # h = shape[0]
        # # w = shape[1]
        #
        #
        roi = tf.reshape(roi, [5,])
        x, y, w_r, h_r, theta = tf.unstack(roi//16)
        w_r = tf.abs(w_r)
        h_r = tf.abs(h_r)
        # theta = tf.where(tf.less_equal(theta, -90.0//16), tf.zeros_like(theta), tf.abs(theta))

        theta = theta * 16 / 180 * pi
        ...
        # xmin = tf.subtract(x, tf.divide(h, 2))
        # ymin = tf.subtract(y, tf.divide(w, 2))
        # xmin = tf.abs(xmin)
        # ymin = tf.abs(ymin)
        #
        #
        ...
        h_r = tf.cast(h_r, tf.int32)
        w_r = tf.cast(w_r, tf.int32)

        #x, y, w_r, h_r, = x//16, y//16, w_r//16, h_r//16
        feature_maps = tf.squeeze(feature_maps,axis=0)
        feature_maps = tf.image.resize_image_with_crop_or_pad(feature_maps, 80, 80)

        crop_features = tf.image.crop_to_bounding_box(feature_maps, ymin + 8, xmin + 8, h+1, w +1 )
        pad_crop_feature = tf.image.resize_image_with_crop_or_pad(crop_features, 28, 28)
        rotate_feature = tf.contrib.image.rotate(pad_crop_feature, theta)
        # shape = tf.cast(tf.shape(rotate_feature), tf.int32)
        # new_h, new_w = shape[0], shape[1]
        rotate_feature = tf.cast(rotate_feature, tf.float32)

        #roi_feature = tf.image.crop_to_bounding_box(rotate_feature, (new_h - h_r)//2, (new_w - w_r)//2, h_r + 1, w_r + 1)
        roi_feature = tf.image.crop_to_bounding_box(rotate_feature, 7, 7, 14,14)
        roi_feature = tf.concat(roi_feature, axis=0)

        return roi_feature
    def RROI(self, feature_maps, rois ):
        with tf.variable_scope('rotate_roi'):
            rroi = tf.map_fn(lambda roi: self.rotate_roi(feature_maps, roi), rois)
            # rroi = tf.py_func(boxes_utils.RROI_, inp=[feature_maps, rois, rois_h], Tout = tf.float32)
            #
            # rroi = tf.reshape(rroi, [-1,-1,-1, 1024])
            roi_features = slim.max_pool2d(rroi,
                                           [cfgs.ROI_POOL_KERNEL_SIZE, cfgs.ROI_POOL_KERNEL_SIZE],
                                           stride=cfgs.ROI_POOL_KERNEL_SIZE)
    ...
    def build_fastrcnn(self, feature_to_cropped, rois, img_shape, scope):
        ...
        with tf.variable_scope('Fast-RCNN_{}'.format(scope)):
            # 5. ROI Pooling
            if scope == 'stage2':# or scope == 'stage3'  :
                with tf.variable_scope('rotate_roi_pooling'):
                    #rois_h = boxes_utils.get_horizen_minAreaRectangle(rois, with_label=False)

                    pooled_features = self.RROI(feature_maps=feature_to_cropped, rois = rois)
            else:
            ...
            with slim.arg_scope([slim.fully_connected], weights_regularizer=slim.l2_regularizer(cfgs.WEIGHT_DECAY)):
                if not scope == 'stage3':
                    ...
                    bbox_pred_ins = tf.reshape(bbox_pred, [-1, cfgs.CLASS_NUM+1, 5])
                    ...
                    bbox_pred_fliter = tf.reshape(tf.gather_nd(bbox_pred_ins, keep_inds), [-1, 5])
    ...
    def add_roi_batch_img_smry_rotate(self, img, rois, labels, scope):
        positive_roi_indices = tf.reshape(tf.where(tf.greater_equal(labels, 1)), [-1])
        ...
        pos_in_img = show_box_in_tensor.draw_box_with_color_rotate(img_batch=img,
                                                        boxes=pos_roi, text='1')
        neg_in_img = show_box_in_tensor.draw_box_with_color_rotate(img_batch=img,
                                                        boxes=neg_roi, text='0')
        tf.summary.image('pos_rois_{}'.format(scope), pos_in_img)
        tf.summary.image('neg_rois_{}'.format(scope), neg_in_img)
        ...
        with tf.variable_scope('build_loss') as sc:
            ...
            with tf.variable_scope('FastRCNN_loss'):
                if not cfgs.FAST_RCNN_MINIBATCH_SIZE == -1:
                    bbox_loss = losses_old.smooth_l1_loss_rcnn_r(bbox_pred=bbox_pred,
                                                               bbox_targets=bbox_targets,
                                                               label=labels,
                                                               num_classes=cfgs.CLASS_NUM + 1,
                                                               sigma=cfgs.FASTRCNN_SIGMA)
                    stage2_bbox_loss = losses_old.smooth_l1_loss_rcnn_r(bbox_pred=stage2_bbox_pred,
                                                                      bbox_targets=stage2_bbox_targets,
                                                                      label=stage2_labels,
                                                                      num_classes=cfgs.CLASS_NUM + 1,
                                                                      sigma=cfgs.FASTRCNN_SIGMA)
                    stage1_bbox_loss = losses_old.smooth_l1_loss_rcnn_r(bbox_pred=stage1_bbox_pred,
            ...
            loss_dict = {
                ...
                "stage1": stage1_bbox_loss,
                "stage2": stage2_bbox_loss
            }
        return loss_dict

    def build_whole_detection_network(self, input_img_batch, gtboxes_h_batch, gtboxes_r_batch):
        ...
        if self.is_training:
            ...
            with tf.control_dependencies([rpn_labels]):
                with tf.variable_scope('sample_RCNN_minibatch_stage1'):
                    ...
                    stage1_bbox_targets = tf.reshape(stage1_bbox_targets, [-1, 5*(cfgs.CLASS_NUM+1)])
        ...
        stage1_bbox = self.postprocess_cascade(rois=stage1_rois,
                                               ...
                                               scope='stage1', five = False)




        ###################     pb test
        #stage1_bbox = encode_and_decode.decode_boxes_rotate(encode_boxes=stage1_bbox_pred_fliter, reference_boxes=stage1_rois, five=False)
        ...
        if self.is_training:
            ...
            overlaps = iou_rotate.iou_rotate_calculate(stage1_bbox, gtboxes_r_batch[:, :-1], use_gpu=True, gpu_id=0)
        ...
        if self.is_training:
            with tf.control_dependencies([stage1_bbox]):
                with tf.variable_scope('sample_RCNN_minibatch_stage2'):
                    ...
                    stage2_bbox_targets = tf.reshape(stage2_bbox_targets, [-1, 5*(cfgs.CLASS_NUM+1)])
                    self.add_roi_batch_img_smry_rotate(input_img_batch, stage2_rois, stage2_labels, 'stage2')
        else:
        ...
        scale_min = tf.constant(0,dtype=tf.float32)
        scale_max = tf.constant(1024, dtype=tf.float32)
        stage2_rois_h = boxes_utils.get_horizen_minAreaRectangle(stage2_rois, with_label=False)
        #stage2_rois_h = tf.abs(stage2_rois_h)
        #scale = tf.sqrt(tf.multiply(stage2_rois[:, 2], stage2_rois[:, 3] ))
        ind1 = tf.equal(tf.greater_equal(stage2_rois_h[:, 0], scale_min), tf.less(stage2_rois_h[:, 0], scale_max))
        ind2 = tf.equal(tf.greater_equal(stage2_rois_h[:, 1], scale_min), tf.less(stage2_rois_h[:, 1], scale_max))
        ind3 = tf.equal(tf.greater_equal(stage2_rois_h[:, 2], scale_min), tf.less(stage2_rois_h[:, 2], scale_max))
        ind4 = tf.equal(tf.greater_equal(stage2_rois_h[:, 3], scale_min), tf.less(stage2_rois_h[:, 3], scale_max))
        index = ind1 & ind2 & ind3 & ind4
        index1 = tf.where(index)
        stage2_rois = tf.gather(stage2_rois, index1)
        stage2_rois = tf.reshape(stage2_rois, [-1, 5])
        if self.is_training:
            stage2_bbox_targets  = tf.gather(stage2_bbox_targets, index1)
            stage2_bbox_targets = tf.reshape(stage2_bbox_targets, [-1, 80])
            stage2_labels = tf.gather(stage2_labels, index1)
            stage2_labels = tf.reshape(stage2_labels, [-1, ])


        # stage2_rois_h = boxes_utils.get_horizen_minAreaRectangle(stage2_rois, with_label=False)  ##斜变正
        ...
        if self.is_training:
            overlaps = iou_rotate.iou_rotate_calculate(stage2_bbox, gtboxes_r_batch[:, :-1], use_gpu=True, gpu_id=0)
        ...
        if self.is_training:
            with tf.control_dependencies([stage2_bbox]):
                with tf.variable_scope('sample_RCNN_minibatch_stage3'):
                    ...
                    stage3_bbox_targets = tf.reshape(stage3_bbox_targets, [-1, 5*(cfgs.CLASS_NUM+1)])
                    self.add_roi_batch_img_smry_rotate(input_img_batch, stage3_rois, stage3_labels, 'stage3')
        else:
        ...
        # scale_min = tf.constant(0, dtype=tf.float32)
        # scale_max = tf.constant(1024, dtype=tf.float32)
        # stage3_rois_h = boxes_utils.get_horizen_minAreaRectangle(stage3_rois, with_label=False)
        # # stage2_rois_h = tf.abs(stage2_rois_h)
        # # scale = tf.sqrt(tf.multiply(stage2_rois[:, 2], stage2_rois[:, 3] ))
        # ind1 = tf.equal(tf.greater_equal(stage3_rois_h[:, 0], scale_min), tf.less(stage3_rois_h[:, 0], scale_max))
        # ind2 = tf.equal(tf.greater_equal(stage3_rois_h[:, 1], scale_min), tf.less(stage3_rois_h[:, 1], scale_max))
        # ind3 = tf.equal(tf.greater_equal(stage3_rois_h[:, 2], scale_min), tf.less(stage3_rois_h[:, 2], scale_max))
        # ind4 = tf.equal(tf.greater_equal(stage3_rois_h[:, 3], scale_min), tf.less(stage3_rois_h[:, 3], scale_max))
        # index = ind1 & ind2 & ind3 & ind4
        # index1 = tf.where(index)
        # stage3_rois = tf.gather(stage3_rois, index1)
        # stage3_rois = tf.reshape(stage3_rois, [-1, 5])
        # stage3_rois_h = boxes_utils.get_horizen_minAreaRectangle(stage3_rois, with_label=False)
        # if self.is_training:
        #     stage3_bbox_targets = tf.gather(stage3_bbox_targets, index1)
        #     stage3_bbox_targets = tf.reshape(stage3_bbox_targets, [-1, 80])
        #     stage3_labels = tf.gather(stage3_labels, index1)
        #     stage3_labels = tf.reshape(stage3_labels, [-1, ])
        
        
        stage3_rois_h = boxes_utils.get_horizen_minAreaRectangle(stage3_rois, with_label=False)  ##斜变正
        ...
        if not self.is_training:
            with slim.arg_scope([slim.conv2d, slim.conv2d_in_plane,
                ...
                return self.postprocess_fastrcnn_r(rois=stage3_rois, bbox_ppred=stage3_bbox_pred, scores=cls_prob, img_shape=img_shape, scope='stage3')
        else:
            '''
            ...
            final_bbox, final_scores, final_category = self.postprocess_fastrcnn_r(rois=stage3_rois,
