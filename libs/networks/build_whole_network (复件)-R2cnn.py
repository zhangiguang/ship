...
class DetectionNetwork(object):
    ...
    def roi_pooling(self, feature_maps, rois, img_shape):
        ...
        return roi_features
    def rotate_roi(self, featuremap, rois_h, rois):
        '''

        :param featuremap: [-1, 64, 64, 1024]
        :      rois_h: [xmin, ymin, xmax, ymax]
        :param rois: [x, y, w, h, theta]
        :return: roi_features
        '''
        with tf.variable_scope('rotate_roi'):
            xmin, ymin, xmax, ymax = tf.unstack(rois_h//cfgs.ANCHOR_STRIDE, axis=1)
            w = tf.cast(xmax - xmin, tf.int32)
            h = tf.cast(ymax - ymin, tf.int32)



            x, y, w_r, h_r, theta = tf.unstack(rois//cfgs.ANCHOR_STRIDE, axis=1)
            theta = theta * cfgs.ANCHOR_STRIDE
            #x, y, w_r, h_r, = x//16, y//16, w_r//16, h_r//16

            crop_features = tf.image.crop_to_bounding_box(featuremap, ymin, xmin, h, w)
            pad_crop_feature = tf.image.resize_image_with_crop_or_pad(crop_features, 2 * h, 2 * w)
            rotate_feature = tf.contrib.image.rotate(pad_crop_feature, theta/180*pi)
            shape = tf.cast(tf.shape(rotate_feature))
            new_h, new_w = shape[0], shape[1]
            roi_feature = tf.image.crop_to_bounding_box(rotate_feature, (new_h - h_r)//2, (new_w - w_r)//2, h_r, w_r)
        return roi_feature
